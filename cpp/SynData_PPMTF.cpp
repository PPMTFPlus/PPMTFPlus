...
//  mat_t A = ReadModelParameter(prefix, K, ItrNum, "A");
  mat_t A = ReadModelParameter(prefix, K, ItrNum, ParamA);
...
//  sprintf(buf, "%s_Itr%d.csv", SynTraceFile.c_str(), ItrNum);
  sprintf(buf, "%s_Itr%d_%s.csv", SynTraceFile.c_str(), ItrNum, ParamA.c_str());
  string outfile(buf);
...
int main(int argc, char *argv[]){
  if(argc < 3){
      printf("Usage: SynData_PPMTF [Dataset] [City] ([TraceNum (default:10)] [ParamA (default:A)] [Alp (default:200)] [MaxNumTrans (default:100)] [MaxNumVisit (default:100)] [ItrNum (default:100)])\n");
      return -1;
  } 
