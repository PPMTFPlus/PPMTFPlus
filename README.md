...
Note that the purpose of this code is to generate **datasets** for an anonymization contest. To obtain the contest results of our paper (e.g., Figures 8 and 9), please download the datasets, sample programs, and submitted files by all teams (~1.0GB zipped) from the website of [PWSCup 2019](https://www.iwsec.org/pws/2019/cup19_e.html) and see README files therein.
...
NOTE: To run our code using Docker files, we used a laptop with Windows 11 Enterprise, Intel i7-1185G7 (3.00GHz, 4 cores), and 32GB RAM. It required about 200MB memory and about 15 minutes in our environment.
...
NOTE: (1), (2), and (3) are for reproducing results in PF. They can be run using Docker Files. (4) and (5) are for reproducing results in FS. They are not covered by Docker Files.
...
NOTE: In (2), we explain how to obtain the POI file (POI_TK.csv) and the trace file (traces_TK.csv) from the [SNS-based people flow data](https://nightley.jp/archives/1954/). The website is written in Japanese, and you need to input your information to download the dataset. However, both the POI and trace files have already been included in data/PF. Therefore, you can skip (2), and in that case, you do not need to download the dataset.
